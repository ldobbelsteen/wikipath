...
export const PathsGraph = (props: { paths?: Paths | "loading" }) => {
  ...
  const text =
    paths === "loading" || paths === undefined
      ...
      : paths.pathCount === 0
        ? "No paths found"
        : `Found ${paths.pathCount} ${
            paths.pathCount === 1 ? "path" : "paths"
          } of degree ${paths.pathLengths}.${
            paths.pathCount > paths.paths.length
              ? ` Only ${paths.paths.length} of them are shown below.`
              : ""
          }`;
  ...
  useEffect(() => {
    ...
    if (paths === "loading" || paths === undefined || paths.pathCount === 0) {
      return;
    }
